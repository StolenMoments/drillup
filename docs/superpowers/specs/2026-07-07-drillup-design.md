...
- 렌더링 시 보기는 서버에서 셔플하여 내려준다.
- 출제 응답에는 각 보기의 원본 인덱스(`original_index`)를 포함하고, 답안 제출 시 이 원본 인덱스를 보낸다.
- `answer_index`는 DB payload 기준 원본 보기 인덱스이며, 출제 응답에는 포함하지 않는다.
...
- MCQ 출제 시 보기는 서버에서 셔플하여 내려주되 제출용 원본 인덱스만 포함한다.
